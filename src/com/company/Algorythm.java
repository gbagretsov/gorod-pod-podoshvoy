      ...
      //  if( Dijkstra.get("tl_0")>= 0)
        ...
        for (Integer i=1; i< City.length; i++) {
            if(City[icur][i] > 0) {
              ...
              Integer dijkstra_i = Dijkstra.get("tl_18")+1;
        ...
        for (Integer i=0; i<City.length; i++){
                Dijkstra = dijkstra(i, City);
                if (( City[icur][i] > 0)&&(!path.contains("tl_".concat(i.toString()))) && ((Dijkstra.get("tl_18")+1) <= max_way)) {
                k = Traffic_NextTL.get("tl_".concat(i.toString()))*2 + (Dijkstra.get("tl_18")+1)*3 ;
                if ((k < min_koef)
